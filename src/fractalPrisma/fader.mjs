    ...
    fade: (
        function(ctx, width, heigth) {
            ...
            for (var i = 0; i < imageData.data.lenght; i+= 4) {
                imageData.data[i] = Math.min(imageData.data[i] - this.deltaFade, 0);
                imageData.data[i+1] = Math.min(imageData.data[i+1] - this.deltaFade, 0);
                imageData.data[i+2] = Math.min(imageData.data[i+2] - this.deltaFade, 0);
            }
